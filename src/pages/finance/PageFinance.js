...
export default class PageFinance extends Component {
  ...
  componentDidMount() {
    ...
    // this.dispatch("fetchEchartOption", {'vo.id': 11}); //获取echart
    ...
    this.dispatch("fetchDataAccord");//获取财务的类型
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      columns: [    
        ...
        { dataKey: "sale_money", title: "主营利润", align: "center"},
        { dataKey: "lm", title: "同比数据", align: "center"},
        { dataKey: "ly", title: "环比数据", align: "center"},
        { dataKey: "lm_money", title: "同比利润", align: "center"},
        { dataKey: "ly_money", title: "环比利润", align: "center"}
      ]
    };
        ...
        {accord.map(item => (
          <div className="t-LH44 t-FBH t-FBAC">
            <div
              ...
              onClick={this.clickFetchType.bind(this, item.name)}
            >
            {item.name}
            </div>
          </div>
...
subPop(options) {
  ...
  if(!this.state.popOptions.isAll){
    this.dispatch("fetchEchartOption", {
      ...
      'name':this.state.popOptions.maccord
    }); //获取echart
    this.dispatch("fetchDataTable",{
      startTime:this.state.popOptions.startDate,
      endTime:this.state.popOptions.endDate,
...
clickFetchOrganization() {
  ...
  render() {
    ...
    const { defaultItems, option, data, columns } = t.state;
    const imgTitle = <img>123</img>;
    return (
